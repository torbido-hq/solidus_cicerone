...
RSpec.describe SolidusCicerone::PostEventsJob do
  ...
  it "loads an order by id through Spree" do
    order = SolidusCicerone::SpecFixtures::Order.new(
      id: 7,
      number: "R123",
      user_id: 12,
    ...
    stub_const("Spree::Order", Class.new { define_singleton_method(:find_by) { |**| order } })
    stub_request(:post, "http://cicerone.test/events")
      .to_return(status: 202, body: { "accepted" => 1 }.to_json)
  ...
  it "skips a missing order id" do
    stub_const("Spree::Order", Class.new { define_singleton_method(:find_by) { |**| nil } })
